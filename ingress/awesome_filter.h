...
#define AWESOME_FILTER_STATS(COUNTER) \
    COUNTER(ok)                       \
    COUNTER(denied)                   \
    COUNTER(error)                    \
 ...
 */
struct AwesomeFilterStats
{
    AWESOME_FILTER_STATS(GENERATE_COUNTER_STRUCT)
};
/**
...
public:
    ...
    const AwesomeFilterStats &stats() const { return stats_; }

private:
    AwesomeFilterStats generateStats(const std::string &prefix, Stats::Scope &scope)
    {
        const std::string final_prefix = prefix + "ingress.";
        return {AWESOME_FILTER_STATS(POOL_COUNTER_PREFIX(scope, final_prefix))};
    }
